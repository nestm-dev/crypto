...
function testIntegrationConsumer(tarball) {
	...
	writeFileSync(
		...
		[
			...
			'const fileEngine = new entry5.FileCipherEngine({ defaultProvider: "packed-file", providers: [{ name: "packed-file", provider: new entry0.AesKeyRingProvider({ activeKeyId: "packed-file-key", keys: { "packed-file-key": generateKeySync("aes", { length: 256 }) } }) }], maxPlaintextBytes: 1024n });',
			'const fileAad = new TextEncoder().encode("packed.file");',
			'const encryptedFile = await fileEngine.encrypt((async function* () { yield new TextEncoder().encode("packed file"); })(), { aad: fileAad });',
			"const encryptedFileBytes = new Uint8Array(await new Response(encryptedFile.encrypted).arrayBuffer());",
			"await encryptedFile.completion;",
