...
const tenantDevelopmentDependency = packageJson.devDependencies?.["@nestm/tenant"];
if (
	typeof tenantDevelopmentDependency !== "string" ||
	tenantDevelopmentDependency.startsWith("file:") ||
	tenantDevelopmentDependency.startsWith("link:") ||
	tenantDevelopmentDependency.startsWith("workspace:")
) {
	throw new Error(
		"Replace the local @nestm/tenant development link with the exact published integration release before publishing.",
	);
}
...
let publishResult;
try {
	publishResult = spawnSync(process.execPath, publishArguments, { stdio: "inherit" });
} finally {
